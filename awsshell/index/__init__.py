...
contains code for generating the a "reverse index" derived from boto3 docs.  This
is primarily done while we explore to what extent the boto3 resource models have
the information we need.  To be fair, the resource models were never intended to
be used in the mechanism in which we're using them, so it's very possible we may
need to enhance the models by hand.  This is why we pre-generate the index
for server side completion up front so we can add hand modifications if needed.
It's very possible that we may be able to come up with something sufficiently
sophisticated enough where we can change this index to by autogenerated on
demand like the other two.
